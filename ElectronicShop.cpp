/*Problem:
		Given two vectors of usb prices and keyboard prices, find the combination which is the closest to the budget
*/
...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
	...
	for (int i = 0; i < keyboards.size(); i++) {
		for (int j = 0; j < drives.size(); j++) {
			...
			else if ((keyboards[i] + drives[j]) < b && (keyboards[i] + drives[j]) > money) {
